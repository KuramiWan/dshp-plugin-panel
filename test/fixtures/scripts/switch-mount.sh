#!/usr/bin/env bash
#
# dshp-skill-panel 本地挂载切换脚本（归档版，2026-08-26 移至 test 分支保存）
#
# 在「开发模式」（link 到工作区，改动即生效）与「真实使用模式」（用已安装/已发布的
# 真实副本）之间切换 web profile 里挂载的 @super_camel/dsh-skill-panel。
#
# 关于本仓的归档说明：
# 本脚本原属 dshp-skill-panel 主仓 main 分支（archive 模式：主仓 .gitignore 排除、不提交）。
# 2026-08-26 随 fixtures 一起迁到 test 分支的 test/fixtures/scripts/，仅作归档保存、
# 不作开发调用。test 分支的 DSH test profile 接入不依赖本脚本。
#
