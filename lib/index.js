...
*/
/** 面板自身包名与行 id（禁止停）。 */
const PANEL_PACKAGE = "@kuramiwan/dsh-skill-panel";
...
function resolveProfileDir() {
	...
	if (existsSync(profilesDir)) for (const name of readdirSync(profilesDir)) {
		...
		try {
			if (readFileSync(patch, "utf8").includes("@kuramiwan/dsh-skill-panel")) return join(profilesDir, name);
		} catch {}
	}
	...
	*/
	list(agent) {
		...
		for (const fiber of fibers) {
			...
			const source = isSelf ? "patch" : isMcp ? "mcp" : patchIds.has(id) ? "patch" : "core";
	...
	*/
	...
	disable(id) {
		...
		if (view.source !== "patch") return {
			ok: false,
			reason: `"${id}" 不是活动 profile 的 patch 行，不可在此停用（bundle 走冷挂载，需改 package.json 后重启）`
		};
	...
	enable(id) {
		...
		if (typeof name !== "string" || name === "") return {
			ok: false,
			reason: `缺少 "${id}" 的包名，无法重建 insert 行（请在状态文件/手动补 name）`
		};
