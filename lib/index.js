...
*/
function moveSkill(poolRoot, name, group) {
	if (!isValidSkillName(name)) return {
		ok: false,
		reason: `非法技能名 "${name}"`
	};
	if (group !== void 0 && group !== "" && !isValidGroupName(group)) return {
		ok: false,
		reason: `非法分组名 "${group}"`
	};
	const entry = findPoolEntry(poolRoot, name);
	...
	const targetDir = group === void 0 || group === "" ? localRoot : join(localRoot, group);
	const target = join(targetDir, name);
	if (current === target) return {
		ok: true,
		name,
		...group === void 0 || group === "" ? {} : { group }
	};
	...
	try {
		...
		return {
			ok: false,
			reason: `移动失败：${error instanceof Error ? error.message : String(error)}`
		};
	}
	return {
		ok: true,
...
var SkillPanelService = class {
	...
	moveSkill(request) {
		...
		return {
			ok: true,
			name: result.name,
			...result.group === void 0 ? {} : { group: result.group }
		};
	}
...
var SessionMcpManager = class {
	...
	*/
	async check(agent, name) {
		if (this.cached.find((s) => s.name === name) === void 0) return {
			ok: false,
			reason: `白名单没有 "${name}"`
		};
		...
		let toolCount = 0;
		try {
			const tools = agent.ctx.get.call(agent.ctx, "tools");
			const prefix = `mcp__${conn.serverName}__`;
			toolCount = (tools?.schemas?.() ?? []).filter((s) => typeof s?.name === "string" && s.name.startsWith(prefix)).length;
		} catch {
			toolCount = 0;
		}
