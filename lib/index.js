...
function setSkillTags$1(directory, tags) {
	...
	if (idx >= 0) {
		lines[idx] = tagLine;
		let j = idx + 1;
		while (j < lines.length && /^\s*-\s/.test(lines[j] ?? "")) lines.splice(j, 1);
...
function groupByFirstTag(items, ungroupedKey) {
...
//#endregion
//#region src/agent.ts
function requireAgent(exec, label) {
	if (exec.agent === void 0) throw new Error(`${label} require a calling agent`);
	return exec.agent;
}
//#endregion
//#region src/tools.ts
...
function applySessionSkillTools(ctx, config) {
	...
	const agentOf = (exec) => requireAgent(exec, "session_skill tools");
...
function applySessionMcpTools(ctx, config) {
	...
	const agentOf = (exec) => requireAgent(exec, "session_mcp tools");
	ctx.effect(() => ctx.tools.register(defineTool({
...
var SkillPanelService = class {
		...
		try {
			...
			switch (method) {
				...
				case "pluginList":
					...
					break;
				case "pluginToggle":
					result = await this.pluginToggle(payload);
					break;
				case "pluginInstall":
					result = this.pluginInstall(payload);
					break;
				case "pluginPromote":
					result = this.pluginPromote(payload);
					break;
				case "sessions":
					result = this.sessions(payload);
					break;
				default:
					this.send(res, 404, {
						ok: false,
	...
	pluginList(request) {
		...
		return { plugins: this.plugins.list(agent).map((p) => ({
			...
			isSelf: p.isSelf,
			...p.pendingRestart === void 0 ? {} : { pendingRestart: p.pendingRestart },
			...p.packageName === void 0 ? {} : { packageName: p.packageName },
			...p.mcp === void 0 ? {} : { mcp: p.mcp }
		})) };
	}
	/** 启停一个插件行（patch/bundle 写组合层；mcp 行连接/断开会话）。 */
	async pluginToggle(request) {
		const agent = this.agentOf(request.sessionId);
		const result = request.enabled ? await this.plugins.enable(request.id, agent) : await this.plugins.disable(request.id, agent);
		if (!result.ok) return {
			ok: false,
			reason: result.reason
		};
	...
	*/
	sessions(request) {
		...
		return { sessions: want === void 0 || want === "" ? all : all.filter((s) => s.sessionId === want) };
	}
};
//#endregion
//#region src/registry.ts
/** 从 cordis.registry 拍平所有已加载插件 Fiber（含 mcp-client）。 */
function registryFibers(ctx) {
...
*/
function isMcpClientConfig(config) {
	const c = config;
...
var SessionMcpManager = class {
	...
	*/
	discover() {
		...
		for (const fiber of registryFibers(this.context)) if (isMcpClientConfig(fiber.config) && fiber.state === 2) actives.add(fiber.config.serverName);
	...
	*/
	select(name) {
		...
		if (!saved.ok) {
			...
			if (!restored.ok) {
				...
				return {
					ok: false,
					reason: `${saved.reason}（且回滚恢复全局失败：${restored.reason}）`
				};
			}
			return {
				ok: false,
	...
	removeTemplate(name) {
		...
		this.persist();
		const restored = this.restoreGlobalMcp(name);
		if (!restored.ok) this.context.logger("mcp").error(`restore global MCP "${name}" failed: ${restored.reason}`);
		return { ok: true };
	}
	...
	*/
	readDisabledRows() {
		...
		try {
			...
			const out = {};
			for (const [name, val] of Object.entries(data)) if (Array.isArray(val)) out[name] = val.filter((r) => r !== null && typeof r === "object");
			else if (val !== null && typeof val === "object") out[name] = [val];
			return out;
		} catch {
			return {};
		}
	...
	restoreGlobalMcp(serverName) {
		...
		if (rows === void 0 || rows.length === 0) return { ok: true };
		const options = this.readHomePatchOptions();
		const existsInAnyBlock = (row) => {
			const cfg = row?.config;
			const rowServer = typeof cfg?.serverName === "string" ? cfg.serverName : void 0;
			return options.some((opt) => {
				if (opt === null || typeof opt !== "object" || !Array.isArray(opt.insert)) return false;
				return opt.insert.some((r) => {
					const rcfg = r?.config;
					return typeof rcfg?.serverName === "string" && rowServer !== void 0 && rcfg.serverName === rowServer;
				});
			});
		};
	...
	*/
	...
	async connect(agent, name, signal) {
		...
		try {
			...
			return {
				ok: false,
				reason: `连接 "${name}" 失败：${error instanceof Error ? error.message : String(error)}`
			};
		}
		try {
		...
		agent.ctx.effect(() => () => {
			...
			if (conn === void 0 || conn.serverName !== serverName) return;
			conn.dispose();
	...
	async awaitWithSignal(promise, signal) {
		...
		await new Promise((resolve, reject) => {
			const onAbort = () => reject(/* @__PURE__ */ new Error("aborted"));
...
function resolveProfileDir() {
	...
	if (existsSync(profilesDir)) for (const name of readdirSync(profilesDir)) {
		...
		if (readTextFileSync(patch)?.includes("@super_camel/dsh-skill-panel")) return join(profilesDir, name);
	}
...
var PluginManager = class {
	...
	classifySource(isSelf, inPatch, isUserBundle, pendingPromote) {
	...
	*/
	list(agent) {
		...
		for (const fiber of fibers) {
			...
			const rawId = hasName ? fiber.name : `(unnamed #${++unnamedSeq})`;
	...
	writeBundleNames(bundleNames) {
		...
		if (text === void 0) return {
			ok: false,
			reason: "profile package.json not readable"
		};
		try {
	...
	*/
	writePatch(rows) {
		...
		const preservedNonManaged = [];
		const kept = [];
		for (const opt of options) {
			if (opt !== null && typeof opt === "object" && Array.isArray(opt.insert)) {
				for (const row of opt.insert) if (row !== null && typeof row === "object" && typeof row.id !== "string") preservedNonManaged.push(row);
				continue;
			}
			kept.push(opt);
		}
		const nextOptions = [{ insert: [...preservedNonManaged, ...rows] }, ...kept];
		let nextText;
		try {
	...
	async disable(id, agent) {
		const view = this.list(agent).find((v) => v.id === id);
	...
	*/
	promoteToPatch(id) {
		...
		if (view.source !== "bundle") return {
			ok: false,
			reason: `"${id}" 不是 bundle 行，无需提升`
		};
		const name = view.packageName;
		if (typeof name !== "string" || name === "") return {
			ok: false,
			reason: `缺少 "${id}" 的包名，无法提升`
		};
		const bundles = this.readBundleNames().filter((n) => n !== name);
		const rmResult = this.writeBundleNames(bundles);
		if (!rmResult.ok) return {
	...
	*/
	rewriteBundleToPatch(packageName) {
		...
		try {
			const pkg = JSON.parse(text);
			if (pkg.dsh?.bundle === void 0) return { ok: true };
			writeFileSync(pkgFile + ".bak", text, "utf8");
			...
			return {
				ok: false,
				reason: `rewrite ${packageName} failed: ${error instanceof Error ? error.message : String(error)}`
			};
		}
	}
	readSpecs() {
...
const LOG_FILE_NAME = ".dshp-skill-panel.log";
/**
...
function formatConsole(level, name, args) {
...
*/
function installPanelLogging(ctx, options = {}) {
	const file = options.logFile ?? defaultLogFile();
	const recentLimit = options.recentLimit ?? 500;
...
var SessionSkillStore = class {
	...
	logger;
	constructor(poolRoot, logger = consoleLogger) {
