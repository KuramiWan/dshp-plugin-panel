...
export const zh = {
  'nav': '技能面板',
  'page.title': '技能面板',
  'page.subtitle': '浏览技能池，按需引入/移除到当前会话。引入即持久化到 ~/.dsh/skills（重启后仍可用）。与 /skill-* 命令、session_skill_* 工具共享同一份状态。',
  ...
  'notice.persisted': '（已持久化到 ~/.dsh/skills，重启后仍可用）',
  'notice.removed': '已从当前会话移除（~/.dsh/skills 常驻副本保留）',
...
export const en = {
  ...
  'page.subtitle': 'Browse the skill pool and introduce/remove skills for the current session. Introducing persists the skill to ~/.dsh/skills (survives restarts). Shares one state with /skill-* commands and session_skill_* tools.',
  ...
  'notice.persisted': ' (persisted to ~/.dsh/skills, survives restarts)',
  'notice.removed': 'Removed from this session (persistent copy stays in ~/.dsh/skills)',
